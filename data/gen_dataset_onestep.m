...
all_ic	= zeros(n_all, data_length);	% inputs
all_out	= zeros(n_all, data_length);	% labels
...
% Make test sets
test_ic = zeros(n_test_dataset, data_length);  % test inputs
test_out = zeros(n_test_dataset, data_length); % test labels

% 从时间序列的末尾往前取 Take the test sets from the end of the time series moving backward
for i = 1:n_test_dataset
    start_idx	= L - 2 * i * data_length + 1;
    end_idx     = start_idx + data_length - 1;
    test_ic(i, :, :)	= u(start_idx:end_idx, :);
    test_out(i, :, :)	= u(start_idx+tau:end_idx+tau, :);
end
...
save(['Datasets/MG_onestep_train_' num2str(n_data_dataset) 'x' num2str(data_length) '.mat'],'data_ic','data_out','val_ic','val_out');
save(['Datasets/MG_onestep_test_' num2str(n_test_dataset) 'x' num2str(data_length) '.mat'],'test_ic','test_out');