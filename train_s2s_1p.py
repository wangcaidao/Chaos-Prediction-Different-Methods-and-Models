...
from train_utils.eval_1d import eval_s2s_1p_iteration
...
def run(args, config):
    ...
    model = LSTM_1d(modes=config['model']['modes1'], fc_dim=config['model']['fc_dim'], in_dim=1, out_dim=1, length=data_config['length'],
...
def test(config):
    ...
    if 'ckpt' in config['test']:
        ...
        model.load_state_dict(ckpt['model'])
        print('Weights loaded from %s' % ckpt_path)
    eval_s2s_1p_iteration(model, dataloader, device=device)
...
if __name__ == '__main__':
    ...
    i = 1
    if i == 1:
        parser.add_argument('--config_path', type=str, default='configs/S2S_1p/train.yaml', help='Path to the configuration file')
        parser.add_argument('--mode', type=str, default='train', help='train or test')
    if i == 2:
        parser.add_argument('--config_path', type=str, default='configs/S2S_1p/test.yaml', help='Path to the configuration file')
